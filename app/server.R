...
shinyServer(function(input, output) {
  ...
  output$mymap2 <- renderLeaflet({
      ...
      {h<- listings %>% filter(minimum_nights<=1)}
      else if (input$night == 'Weekend')
      {h<- listings %>% filter(minimum_nights<=3)}
      else if (input$night == '10 nights')
      {h<- listings %>% filter(minimum_nights<=10)}  
      else if (input$night == '30 nights')
      {h<- listings %>% filter(minimum_nights<=30)}   
 ...
 #     h <- h %>% filter(rating > input$rating_range[1] && rating<input$rating_range[2])
     ...
     leaflet(h) %>%
      setView(long(input$Neighbor), lat(input$Neighbor), zoom(input$Neighbor))%>%
      addProviderTiles("CartoDB.Positron") %>%
      addCircles(lng = ~longitude, lat = ~latitude, radius = ~room_type, color = ~palc(room_type), stroke = T)
