...
## LOAD DATA
## Page1
processed_cult_data <-read.csv("../data/culture_centers.csv")
processed_rest_data <-read.csv("../data/restaurant_data.csv")
...
shinyServer(function(input, output) {
  ...
  output$map1 <- renderLeaflet({
    map_load <-  rbind(processed_cult_data,processed_rest_data)
    
    if(input$Centers!="All Cultural Centers" && input$Rest_sel!="All Types"){
      map_load<-rbind(processed_cult_data%>% filter(Type == input$Centers),processed_rest_data%>% filter(Type == tolower(input$Rest_sel)))
    }
    else if (input$Centers!="All Cultural Centers" && input$Rest_sel=="All Types"){
      map_load<-rbind(processed_cult_data%>% filter(Type == input$Centers),processed_rest_data)
    }
    else if (input$Centers=="All Cultural Centers" && input$Rest_sel!="All Types"){
      map_load<-rbind(processed_cult_data,processed_rest_data%>% filter(Type == tolower(input$Centers)))
    }
